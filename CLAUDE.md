...
Hero uses `min-height: 50vh` (not 100vh) so content is reachable faster. Minimap is visible from page load — index.html, training.html, and training-economics.html observe both `.hero` and `#pipeline` with a Set-based IntersectionObserver (minimap hides when neither is in viewport, i.e. footer area). economics.html uses a scroll handler with footer position check. Minimap phase labels use descriptive names ("Request Prep", "GPU Compute", etc.) not letter codes (A, B, C).
