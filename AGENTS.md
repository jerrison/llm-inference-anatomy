...
18. **Hero and minimap visibility**: Hero uses `min-height: 50vh` (not 100vh) so content is reachable faster. Minimap is visible from page load via Set-based IntersectionObserver on both `.hero` and `#pipeline` — hides only in footer area when neither is in viewport. Minimap phase labels use descriptive section names ("Request Prep", "GPU Compute", "Unit Economics", etc.) not letter codes (A, B, C).
